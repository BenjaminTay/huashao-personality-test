...
项目已按静态导出配置，`npm run build` 会生成 `out/`，不需要 Node.js 服务端即可托管。仓库包含 GitHub Actions workflow：推送到 `main` 后会自动完成依赖安装、Lint、类型检查、测试、静态构建，并发布到 GitHub Pages。

GitHub Pages 项目站点的地址通常是：

```text
https://<GitHub 用户名>.github.io/<仓库名>/
```

GitHub Pages 只适合当前这种静态前端。若未来加入 API、SSR、登录、数据库或服务端分享卡生成，需要改用具备服务端运行时的托管方案。部署前确认：
